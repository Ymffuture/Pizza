...
// ─── Reaction button ──────────────────────────────────────────────────────────
// ✅ FIX: antd icons (LikeFilled etc.) don't accept a `size` prop the same way
// lucide icons do. Instead of `<Icon size={20}>`, we render them inside a
// wrapping span with a known font-size so the icon scales correctly.
const ReactionButton = ({ icon: Icon, count, active, onClick, label, color }) => (
  <Tooltip title={label}>
    <motion.button
      whileTap={{ scale: 0.9 }}
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-all flex-1 justify-center ${
        active ? color : "text-gray-600 dark:text-gray-400"
      }`}
    >
      {/* ✅ antd icons sized via fontSize wrapper, not size prop */}
      ...
      {images.slice(0, 4).map((img, i) => (
        ...
        >
          <img
            ...
            loading="lazy"
            onError={(e) => { e.target.src = "https://swiftmeta.vercel.app/pp.jpeg"; }}
          />
          {images.length > 4 && i === 3 && (
...
// ─── Share menu ───────────────────────────────────────────────────────────────
const shareMenuItems = (postUrl, title) => [
  {
    key: "copy",
    label: "Copy link",
    icon: <Link2 size={16} />,
  ...
  },
  {
    ...
    onClick: () =>
      window.open(`https://wa.me/?text=${encodeURIComponent(title + " " + postUrl)}`, "_blank"),
  },
];
...
export default function BlogHome() {
  ...
  // ✅ FIX: shareModalVisible → shareModalOpen  (Modal uses `open` not `visible` in AntD 5)
  ...
  // ✅ FIX: `postMenu` now returns the `items` array format required by AntD 5.
  // The old `overlay={<Menu>}` pattern is deprecated — use `menu={{ items }}`.
  const getPostMenuItems = (post) => [
    {
      key: "edit",
      icon: <Pencil size={16} />,
      label: "Edit post",
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Explore free HTML, CSS, JavaScript, React, and Next.js projects." />
      ...
      <main className="min-h-screen bg-[#f0f2f5] dark:bg-[#18191a] pt-20 pb-28">
        <div className="max-w-[680px] mx-auto px-4">
          ...
          >
            <div className="flex items-center gap-3">
              <Avatar size={40} src="https://swiftmeta.vercel.app/pp.jpeg" className="cursor-pointer" />
              <div
          ...
          <InfiniteScroll
            ...
            loader={<PostSkeleton />}
            endMessage={
              <div className="text-center py-8 text-gray-500 text-sm">No more posts to show</div>
            }
          >
            <AnimatePresence>
              {posts.map((post, index) => (
                ...
                >
                  ...
                  <div className="p-4 flex items-start justify-between">
                    ...
                    {/* ✅ FIX: `menu={{ items }}` instead of deprecated `overlay={<Menu>}` */}
                    <Dropdown
                  ...
                  <div className="px-2 py-1 flex items-center justify-between">
                    ...
                    />
                    {/* ✅ FIX: Dropdown uses `menu={{ items }}` */}
                    <Dropdown
                      menu={{
                        items: shareMenuItems(
                          `${window.location.origin}/dashboard/blog/posts/${post._id}`,
                          post.title
                        ),
                      }}
                      placement="topCenter"
        ...
        {/* ✅ FIX: Modal uses `open` not `visible` in AntD 5 */}
        <Modal
          title="Share"
          open={shareModalOpen}
        ...
        >
          {selectedPost && (
            <div className="space-y-2 py-2">
              {shareMenuItems(
                `${window.location.origin}/dashboard/blog/posts/${selectedPost._id}`,
                selectedPost.title
              ).map((item) =>
                item.type === "divider" ? (
                  <hr key="divider" className="border-gray-200 dark:border-gray-700" />
                ) : (
                  <button
                    key={item.key}
                    onClick={() => { item.onClick?.(); setShareModalOpen(false); }}
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <div className="flex-1 relative">
          <div className="flex items-center bg-white dark:bg-[#242526] rounded-full border border-gray-200 dark:border-gray-600 px-3 py-2 focus-within:border-blue-500 transition-colors">
            ...
            />
            <Popover
              content={<EmojiPicker onEmojiClick={(d) => { setText((t) => t + d.emoji); setShowEmoji(false); inputRef.current?.focus(); }} width={300} height={400} />}
              trigger="click"
              open={showEmoji}
      ...
      <div className="space-y-3">
        {(showAll ? comments : comments.slice(0, 2)).map((comment) => (
          <CommentItem key={comment._id} comment={comment} postId={postId} onUpdate={fetchComments} />
        ))}
      </div>
    ...
    <div className="flex gap-2">
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-3 mt-1 ml-2 text-xs text-gray-500 dark:text-gray-400">
          <button onClick={handleLike} className={`font-semibold hover:underline ${isLiked ? "text-blue-500" : ""}`}>
            Like {comment.likes?.length > 0 && `(${comment.likes.length})`}
          </button>
          <button onClick={() => setShowReply(!showReply)} className="font-semibold hover:underline">Reply</button>
            ...
            {comment.replies.map((reply) => (
              <div key={reply._id} className="flex gap-2">
                ...
                <div className="bg-gray-100 dark:bg-[#3a3b3c] rounded-2xl px-3 py-2 inline-block">
                  <span className="font-semibold text-xs text-gray-900 dark:text-white block">{reply.author?.name}</span>
          ...
          <div className="mt-2 flex gap-2">
            ...
            <div className="flex-1 flex gap-2">
              <input
                ...
                autoFocus
              />
              <button onClick={handleReply} disabled={!replyText.trim()} className="p-1.5 bg-blue-500 text-white rounded-full disabled:opacity-50">
                <Send size={12} />
              </button>
            </div>
          </div>
